-- Snapshot of the schema that previously lived as two idempotent SQL blobs
-- (_SCHEMA_SQL, _MIGRATION_SQL) directly in core/database.py, run in full on
-- every app startup. Every statement here is still IF NOT EXISTS / ADD COLUMN
-- IF NOT EXISTS / DROP ... IF EXISTS — safe to apply against a database that
-- already has all of this (which is the case for every existing deployment:
-- this migration's only job on first run there is to get recorded in
-- schema_migrations, not to change anything). Going forward, new schema
-- changes get their own 002_*.sql file instead of growing this one.
...
-- Migrations applied to existing tables (formerly _MIGRATION_SQL)
